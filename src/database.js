...
module.exports = {
	...
	cekLoginAdmin: function (email, pass, callback) {
		var req = [email, pass];
		c.query("SELECT * FROM `test_engineers` WHERE `email`=? AND `password`=? AND role='2'", req, { metadata: true, useArray: true }, function (err, rows) {
			if (err) {
	...
	cekLoginUser: function (email, pass, callback) {
		var req = [email, pass];
		c.query("SELECT * FROM test_engineers WHERE email=? AND password=? AND role='1'", req, { metadata: true, useArray: true }, function (err, rows) {
			if (err) {
				res.status(500).send({ message: "Error 500: Internal Server Error" });
				console.log(err);
				return
	...
	forgotPassword_Admin(req, res, token) {
		...
		c.query("SELECT `name` FROM `test_engineers` WHERE `email`=? AND `id` LIKE 'A%'", [req.email], { metadata: true, useArray: true }, function (err, rows) {
			if (err) {
				res.status(500).send({ message: "Error 500: Internal Server Error" });
				console.log(err);
				return
	...
	forgotPassword_Admin_editPassword: function (req, password, res) {
		const waktu = new Date().toISOString();
		c.query("UPDATE `test_engineers` SET `password`=?, `updated`=? WHERE `email`=? AND `id` LIKE 'A%'", [password, waktu, req.email], { metadata: true, useArray: true }, function (err, rows) {
			if (err) {
				res.status(500).send({ message: "Error 500: Internal Server Error" });
				console.log(err);
				return
	...
	checkAdminRegistered: function (req, res) {
		c.query("SELECT * FROM `test_engineers` WHERE `email`=? AND `id` LIKE 'A%'", [req.email], { metadata: true, useArray: true }, function (err, rows) {
			if (err) {
				res.status(500).send({ message: "Error 500: Internal Server Error" });
				console.log(err);
				return
	...
	setAdminPrivilege: function (req, res) {
		...
		c.query("UPDATE `test_engineers` SET `status`=?, `updated`=? WHERE `id`=? AND `id` LIKE 'A%'", request, { metadata: true, useArray: true }, function (err, rows) {
			if (err) {
				res.status(500).send({ message: "Error 500: Internal Server Error" });
				console.log(err);
				return
